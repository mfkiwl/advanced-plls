...
%     - AKF-AR     : KF with AR augmentation using nwpr adaptation (hard_limited = false)
%     - AHL-KF-AR  : KF with AR augmentation using nwpr adaptation (hard_limited = true)
%     - KF-std     : Standard KF (no AR augmentation; training_scint_model = 'none', no adaptive update)
%     - AKF-std    : Standard KF using nwpr adaptation (hard_limited = false)
%     - AHL-KF-std : Standard KF using nwpr adaptation (hard_limited = true)
%
...
[rx_sig_csm, los_phase, psi_csm] = get_received_signal(L1_C_over_N0_dBHz, 'CSM', doppler_profile, seed, ...
    'S4', S4, 'tau0', tau0, 'simulation_time', simulation_time, 'settling_time', settling_time);
[rx_sig_tppsm, ~, psi_tppsm, diffractive_phase_tppsm, refractive_phase_settled] = get_received_signal(L1_C_over_N0_dBHz, 'TPPSM', doppler_profile, seed, ...
    'tppsm_scenario', cppsm_scenario, 'simulation_time', simulation_time, 'settling_time', settling_time, 'is_refractive_effects_removed', is_refractive_effects_removed_received_signal);
...
training_data_config_tppsm = struct('scintillation_model', 'TPPSM', 'scenario', cppsm_scenario, ...
...
%% Define new adaptive configuration structures

% Example threshold in dB; adjust as needed.
hard_limited_threshold = 35;  
...
adaptive_config_AHL_KF_AR = struct(...
    'measurement_cov_adapt_algorithm', 'nwpr', ...
    'measurement_cov_adapt_algorithm_params', struct('N_nwpr', N_nwpr, 'M_nwpr', M_nwpr), ...
    'states_cov_adapt_algorithm', 'none', ...
    'hl_start_time', 50, ...
    'sampling_interval', sampling_interval, ...
    'hard_limited', struct('is_used', true, 'L1_C_over_N0_dBHz_threshold', hard_limited_threshold));
...
adaptive_config_AKF_std = struct(...
    'measurement_cov_adapt_algorithm', 'nwpr', ...
    'measurement_cov_adapt_algorithm_params', struct('N_nwpr', N_nwpr, 'M_nwpr', M_nwpr), ...
    'states_cov_adapt_algorithm', 'none', ...
    'sampling_interval', sampling_interval, ...
...
adaptive_config_AHL_KF_std = struct(...
    'measurement_cov_adapt_algorithm', 'nwpr', ...
    'measurement_cov_adapt_algorithm_params', struct('N_nwpr', N_nwpr, 'M_nwpr', M_nwpr), ...
    'hl_start_time', 50, ...
    'states_cov_adapt_algorithm', 'none', ...
...
online_mdl_learning_cfg = struct('is_online', false, 'learning_method', 'sliding_window', 'window_size', 2000);
...
[kf_ar_csm, ~]      = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_csm, 'standard', 'CSM', adaptive_config_KF_AR, online_mdl_learning_cfg);
[akf_ar_csm, ~]  = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_csm, 'standard', 'CSM', adaptive_config_AKF_AR, online_mdl_learning_cfg);
[ahl_kf_ar_csm, ~]   = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_csm, 'standard', 'CSM', adaptive_config_AHL_KF_AR, online_mdl_learning_cfg);
...
[kf_std_csm, ~] = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_KF_std, online_mdl_learning_cfg);
[akf_std_csm, ~] = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_AKF_std, online_mdl_learning_cfg);
[ahl_kf_std_csm, ~] = get_kalman_pll_estimates(rx_sig_csm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_AHL_KF_std, online_mdl_learning_cfg);

%% Obtain state estimates for TPPSM
% For TPPSM, the training_scint_model is 'TPPSM' (AR augmented).
[kf_ar_tppsm, ~] = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_tppsm, 'standard', 'TPPSM', adaptive_config_KF_AR, online_mdl_learning_cfg);
[akf_ar_tppsm, ~] = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_tppsm, 'standard', 'TPPSM', adaptive_config_AKF_AR, online_mdl_learning_cfg);
[ahl_kf_ar_tppsm, ~] = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_tppsm, 'standard', 'TPPSM', adaptive_config_AHL_KF_AR, online_mdl_learning_cfg);

% For standard KF estimates with TPPSM received signal, training_scint_model is 'none'.
[kf_std_tppsm, ~] = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_KF_std, online_mdl_learning_cfg);
[akf_std_tppsm, ~] = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_AKF_std, online_mdl_learning_cfg);
[ahl_kf_std_tppsm, ~]  = get_kalman_pll_estimates(rx_sig_tppsm, kf_cfg, init_estimates_none, 'standard', 'none', adaptive_config_AHL_KF_std, online_mdl_learning_cfg);
...
plot_time_series_full(...
    ...
    akf_ar_csm, ...     % AKF-AR (nwpr, hard_limited false, AR)
    ahl_kf_ar_csm, ...  % AHL-KF-AR (nwpr, hard_limited true, AR)
    kf_std_csm, ...     % KF-std (no adaptive update, standard KF)
    akf_std_csm, ...    % AKF-std (nwpr, hard_limited false, standard KF)
    ahl_kf_std_csm, ... % AHL-KF-std (nwpr, hard_limited true, standard KF)
    kf_ar_tppsm, ...
    akf_ar_tppsm, ...
    ahl_kf_ar_tppsm, ...
    kf_std_tppsm, ...
...
%     akf_ar_csm(zoomed_samples,:), ...     % AKF-AR (nwpr, hard_limited false, AR)
%     ahl_kf_ar_csm(zoomed_samples,:), ...  % AHL-KF-AR (nwpr, hard_limited true, AR)
%     kf_std_csm(zoomed_samples,:), ...     % KF-std (no adaptive update, standard KF)
%     akf_std_csm(zoomed_samples,:), ...    % AKF-std (nwpr, hard_limited false, standard KF)
%     ahl_kf_std_csm(zoomed_samples,:), ... % AHL-KF-std (nwpr, hard_limited true, standard KF)
