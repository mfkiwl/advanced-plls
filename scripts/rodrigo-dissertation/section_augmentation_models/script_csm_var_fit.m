...
xlabel('VAR Model Order');
ylabel('Percentage of runs [%]');
title('Optimal VAR Order – Phase');
legend('Location','best');
grid on;

% Export Optimal VAR Order plot & CSV
fig_name = 'optimal_var_order';
...
for i = 1:numel(severities)
    ...
    for mc = 1:num_realizations
        ...
        S_var_phs = 2 * (C_phs/fs) * abs(H_phs).^2;
...
% Export PSD vs. VAR PSD plot & CSV
...
T_psd = table( ...
  ...
  psd_comparison.amplitude.Weak.var_psd,        psd_comparison.amplitude.Moderate.var_psd,        psd_comparison.amplitude.Strong.var_psd, ...
  psd_comparison.phase.Weak.periodogram,       psd_comparison.phase.Moderate.periodogram,       psd_comparison.phase.Strong.periodogram,  ...
  psd_comparison.phase.Weak.var_psd,            psd_comparison.phase.Moderate.var_psd,            psd_comparison.phase.Strong.var_psd, ...
  'VariableNames',{ ...
    'Freq_Hz', ...
    'AmpPer_Weak','AmpPer_Moderate','AmpPer_Strong', ...
    'AmpVAR_Weak','AmpVAR_Moderate','AmpVAR_Strong', ...
    'PhsPer_Weak','PhsPer_Moderate','PhsPer_Strong', ...
    'PhsVAR_Weak','PhsVAR_Moderate','PhsVAR_Strong' } );
