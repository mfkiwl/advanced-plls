...
function [xkk1_TS, Pkk1_TS] = EKF_VAR_Separate_PLL(Px_0_0,x_hat_0_0,yk,Rk,Fk,Qk,M,L,NumSeries,P_rho,P_phi,AmpL1,AmpL2,AmpL5,Aux,C,TopologySelector,AdaptSwitch,CN0L1,CN0L2,CN0L5,T_I)
...
for k = 1:Samples
    ...
    JHk = [JHd,zeros(2*L,M-1),JHrho,zeros(2*L,L*(P_rho-1)),JHphi,zeros(2*L,L*(P_phi-1))];
    ...
    Kk = Pkk1*JHk'*((JHk*Pkk1*JHk' + Rk_adapt)\eye(2*L)); % Kalman Gain computation
